...
import { type SqlLogger, withConnection } from "./db";
import {
...
export function json_object(
  ...
  target: Target,
  tableAlias: string,
  nest: string = `${tableAlias}.`,
): string {
  const result = Object.entries(fields)
    .map(([fieldName, field]) => {
      switch (field.type) {
        case "column":
          return `${escapeString(fieldName)}, ${nest}${escapeIdentifier(
            field.column,
          )}`;
        case "relationship":
          ...
          if (rel === undefined) {
            throw new Error(
              `Couldn't find relationship ${
                field.relationship
              } for field ${fieldName} on target ${JSON.stringify(target)}`,
            );
          }
          return `'${fieldName}', ${relationship(
            all_relationships,
            rel,
            field,
            tableAlias,
        ...
        case "object":
          return `${escapeString(fieldName)}, ${json_object(
            [],
            field.query.fields ?? {},
            target,
            tableAlias,
            `(${nest}${escapeIdentifier(field.column)}).`,
          )}`;
        case "array":
          if (field.field.type === "column") {
            return `${escapeString(fieldName)}, TO_JSONB(${escapeIdentifier(
              field.field.column,
            )})`;
          } else if (field.field.type === "object") {
            return `${escapeString(fieldName)}, TO_JSONB(ARRAY_TRANSFORM(${escapeIdentifier(field.field.column)}, |x| ${json_object(
              [],
            ...
            throw new Error("Array of non-columns not supported yet");
          }
        default:
  ...
  return tag("json_object", `JSONB_BUILD_OBJECT(${result})`);
}
    ...
    switch (expression.type) {
      ...
      case "and":
        const aAnd = expression.expressions.flatMap((x) =>
          generateWhere(x, currentTarget, currentTableAlias),
        );
      ...
      case "exists":
        const joinInfo = calculateExistsJoinInfo(
          relationships,
          expression,
          currentTarget,
          currentTableAlias,
        ...
        const subqueryWhere = generateWhere(
          expression.where,
          tableTarget,
          joinInfo.joinTableAlias,
        );
        const whereComparisons = [
          ...joinInfo.joinComparisonFragments,
          subqueryWhere,
        ].join(" AND ");
        return tag(
          "exists",
          `EXISTS (SELECT 1 FROM ${escapeTargetName(joinInfo.joinTarget)} AS ${
            joinInfo.joinTableAlias
          } WHERE ${whereComparisons})`,
        );
      ...
      case "binary_arr_op":
        ...
        const bopARhsValues = expression.values
...
function calculateExistsJoinInfo(
  ...
  switch (exists.in_table.type) {
    case "related":
      const tableRelationships = find_relationships(
        allRelationships,
        sourceTarget,
      );
      const relationship =
        tableRelationships.relationships[exists.in_table.relationship];
      if (!relationship) {
        throw new Error("relations doesn't contain relation");
      }
      const joinTableAlias = generateTargetAlias(relationship.target);

      const joinComparisonFragments =
        generateRelationshipJoinComparisonFragments(
          relationship,
          sourceTableAlias,
          joinTableAlias,
        );
...
function generateRelationshipJoinComparisonFragments(
  relationship: Relationship,
  sourceTableAlias: string,
  targetTableAlias: string,
): string[] {
  const sourceTablePrefix = `${sourceTableAlias}.`;
  return Object.entries(relationship.column_mapping).map(
    ([sourceColumnName, targetColumnName]) =>
      `${sourceTablePrefix}${escapeIdentifier(
        sourceColumnName,
      )} = ${targetTableAlias}.${escapeIdentifier(targetColumnName as string)}`,
  );
}
...
function generateComparisonColumnFragment(
  ...
  const prefix = path.length === 0 ? currentTablePrefix : queryTablePrefix;
  const selector = getColumnSelector(prefix, comparisonColumn.name);
...
function find_relationships(
  ...
  switch (target.type) {
    case "table":
      for (var i = 0; i < allRelationships.length; i++) {
        ...
        if (
          r.source_table != undefined &&
          tableNameEquals(r.source_table)(target)
        ) {
          return r;
        }
      }
    ...
    case "interpolated":
      for (var i = 0; i < allRelationships.length; i++) {
        ...
        if (
          r.source_interpolated_query != undefined &&
          r.source_interpolated_query == target.id
        ) {
          return r;
        }
      }
  ...
  throw new Error(
    `Couldn't find table relationships for target ${JSON.stringify(
      target,
...
function aggregates_query(
  ...
  const sourceSubquery = `SELECT ${tableAlias}.* FROM ${escapeTargetName(
    target,
  )} AS ${tableAlias} ${orderByJoinClauses} ${whereClause} ${orderByClause} ${limit(
    wLimit,
  )} ${offset(wOffset)}`;
  ...
  const aggregate_pairs = Object.entries(aggregates)
    .map(([k, v]) => {
      switch (v.type) {
        ...
        case "column_count":
          if (v.distinct) {
            return `${escapeString(k)}, COUNT(DISTINCT ${escapeIdentifier(
              v.column,
            )})`;
          } else {
        ...
        case "single_column":
          return `${escapeString(k)}, ${cast_aggregate_function(
            v.function,
          )}(${escapeIdentifier(v.column)})`;
      }
    })
    .join(", ");

  return `'aggregates', (SELECT JSONB_BUILD_OBJECT(${aggregate_pairs}) FROM (${sourceSubquery}))`;
}
...
function target_query( // TODO: Rename as `target_query`
    ...
    fields === null
      ...
      : (() => {
          ...
          if (wOrder === null || wOrder.elements.length < 1) {
            return `FROM ( SELECT ${json_object(
              allRelationships,
              fields,
              target,
              tableAlias,
            )} AS j FROM ${escapeTableName(
              tableName,
            )} AS ${tableAlias} ${whereClause} ${limit(wLimit)} ${offset(
              wOffset,
            )})`;
          } else {
            ...
            return `FROM (SELECT ROW_NUMBER() OVER (PARTITION BY 1 ${orderByClause}) AS ord, ${json_object(
              ...
              tableName,
            )} AS ${tableAlias} ${orderByJoinClauses} ${whereClause} ${limit(
              wLimit,
            )} ${offset(wOffset)})`;
          }
        })();
  ...
  return tag(
    "table_query",
    `(SELECT JSONB_BUILD_OBJECT(${[...fieldSelect, ...aggregateSelect].join(
      ", ",
    )}) ${fieldFrom})`,
  );
}
  ...
  const [limit, aggregatesLimit] =
    r.relationship_type === "object"
      ...
      : [
          ...
          coerceUndefinedToNull(field.query.aggregates_limit),
        ];
...
function bop_op(o: BinaryComparisonOperator): string {
  ...
  switch (o) {
    ...
    case "_regexp":
      result = "REGEXP";
      break; // TODO: Have capabilities detect if REGEXP support is enabled
    case "_and":
      result = "AND";
      break;
    case "_or":
...
function orderBy(
  ...
  const joinInfos = Object.entries(orderBy.relations).flatMap(
    ([subrelationshipName, subrelation]) =>
      generateOrderByJoinClause(
        allRelationships,
        orderBy.elements,
        [],
        subrelationshipName,
  ...
  const orderByFragments = orderBy.elements.map((orderByElement) => {
    const targetTableAlias =
      orderByElement.target_path.length === 0
        ? queryTableAlias
        : (() => {
            const joinInfo = joinInfos.find(
              (joinInfo) =>
                joinInfo.joinTableType ===
                  getJoinTableTypeForTarget(orderByElement.target) &&
                stringArrayEquals(joinInfo.relationshipPath)(
                  orderByElement.target_path,
                ),
            );
            if (joinInfo === undefined)
              throw new Error("Can't find a join table for order by target."); // Should not happen 😉
            return joinInfo.tableAlias;
          })();

    const targetColumn = getOrderByTargetAlias(
      targetTableAlias,
...
function getJoinTableTypeForTarget(
  orderByTarget: OrderByTarget,
): OrderByJoinTableType {
...
function generateOrderByJoinClause(
  ...
  if (!relationship) {
    throw new Error("Relationships doesn't contain relation");
  }

  const orderByElements = allOrderByElements.filter(
    byTargetPath(relationshipPath),
  );
  const columnTargetsExist = orderByElements.some(
    (element) => getJoinTableTypeForTarget(element.target) === "column",
  );
  const aggregateElements = orderByElements.filter(
    (element) => getJoinTableTypeForTarget(element.target) === "aggregate",
  );

  const [columnTargetJoin, subrelationJoinInfo] = (() => {
    const subrelationsExist =
      Object.keys(orderByRelation.subrelations).length > 0;
    if (columnTargetsExist || subrelationsExist) {
      const columnTargetJoin = generateOrderByColumnTargetJoinInfo(
        allRelationships,
        relationshipPath,
        relationship,
        sourceTableAlias,
        orderByRelation.where,
      );

      const subrelationJoinInfo = Object.entries(
        orderByRelation.subrelations,
  ...
  const aggregateTargetJoin =
    aggregateElements.length > 0
      ? [
          generateOrderByAggregateTargetJoinInfo(
            allRelationships,
            relationshipPath,
            relationship,
            sourceTableAlias,
  ...
  return [...columnTargetJoin, ...aggregateTargetJoin, ...subrelationJoinInfo];
}

const byTargetPath =
  (relationshipPath: string[]) =>
  (orderByElement: OrderByElement): boolean =>
    stringArrayEquals(orderByElement.target_path)(relationshipPath);
...
function generateOrderByColumnTargetJoinInfo(
  ...
  const joinComparisonFragments = generateRelationshipJoinComparisonFragments(
    relationship,
    sourceTableAlias,
    targetTableAlias,
  );
  const whereComparisons = whereExpression
    ? [
        where_clause(
          allRelationships,
          whereExpression,
    ...
    : [];
  const joinOnFragment = [...joinComparisonFragments, ...whereComparisons].join(
    " AND ",
  );

  const joinClause = tag(
    "columnTargetJoin",
    `LEFT JOIN ${escapeTargetName(
      relationship.target,
    )} AS ${targetTableAlias} ON ${joinOnFragment}`,
  );
  return {
...
function generateOrderByAggregateTargetJoinInfo(
  ...
  const aggregateColumnsFragments = aggregateElements.flatMap((element) => {
    switch (element.target.type) {
      case "column":
        return [];
      case "star_count_aggregate":
        return `COUNT(*) AS ${getOrderByTargetAlias(
          targetTableAlias,
          element.target,
        )}`;
      case "single_column_aggregate":
        return `${cast_aggregate_function(
          element.target.function,
        )}(${escapeIdentifier(
          element.target.column,
        )}) AS ${getOrderByTargetAlias(targetTableAlias, element.target)}`;
      default:
        unreachable(element.target["type"]);
    }
  });
  const joinColumns = Object.values(
    relationship.column_mapping as Record<string, string>,
  ).map(escapeIdentifier);
  const selectColumns = [...joinColumns, aggregateColumnsFragments];
  const whereClause = whereExpression
    ? `WHERE ${where_clause(
        allTableRelationships,
        whereExpression,
        relationship.target,
        subqueryTableAlias,
      )}`
    : "";
  const aggregateSubquery = `SELECT ${selectColumns.join(
    ", ",
  )} FROM ${escapeTargetName(
    relationship.target,
  )} AS ${subqueryTableAlias} ${whereClause} GROUP BY ${joinColumns.join(
    ", ",
  )}`;
  ...
  const joinClause = tag(
    "aggregateTargetJoin",
    `LEFT JOIN (${aggregateSubquery}) AS ${targetTableAlias} ON ${joinOnFragment}`,
  );
  return {
...
function getOrderByTargetAlias(
  table: string,
  orderByTarget: OrderByTarget,
): string {
  switch (orderByTarget.type) {
    case "column":
      return getColumnSelector(table, orderByTarget.column);
    case "star_count_aggregate":
      return `${table}.${escapeIdentifier("__star_count__")}`;
    case "single_column_aggregate":
      return `${table}.${escapeIdentifier(
        `__${orderByTarget.function}_${orderByTarget.column}__`,
      )}`;
    default:
      return unreachable(orderByTarget["type"]);
  }
...
function where(
  ...
  const joinArray = joinInfo
    ? Object.entries(joinInfo.columnMapping).map(
        ([sourceColumn, targetColumn]) =>
          `${joinInfo.sourceTableAlias}.${escapeIdentifier(
            sourceColumn,
          )} = ${queryTableAlias}.${escapeIdentifier(targetColumn)}`,
      )
    : [];
...
function query(request: QueryRequest): string {
  const cte =
    request.interpolated_queries == null
      ? ""
      : cte_block(request.interpolated_queries);
 ...
 * ```
 */
function foreach_ids_table_value(
  foreachIds: Record<string, ScalarValue>[],
): string {
  const columnNames = Object.keys(foreachIds[0] ?? {});

  const columns = columnNames.map(
    (name) =>
      `value ->> ${escapeString("$." + name)} AS ${escapeIdentifier(name)}`,
  );
  const jsonData = foreachIds.map((ids) =>
    mapObject(ids, ([column, scalarValue]) => [column, scalarValue.value]),
  );

  return tag(
    "foreach_ids_table_value",
    `SELECT ${columns} FROM JSON_EACH(${escapeString(
...
function foreach_query(
  ...
  const foreachTableRelationship: TableRelationships = {
    ...
    relationships: {
      [foreachRelationshipName]: {
        ...
        column_mapping: mapObject(
          foreachIds[0] ?? {},
          ([columnName, _scalarValue]) => [columnName, columnName],
        ),
      },
    },
  };
  ...
  return tag(
    "foreach_query",
    `WITH ${escapeTableName(
      foreachTableName,
    )} AS (${foreachIdsTableValue}) SELECT ${tableSubquery} AS data`,
  );
}
...
export async function queryData(
  ...
  return await withConnection(config, sqlLogger, async (db) => {
    ...
    try {
      ...
      console.error("Error parsing query results:", results);
    }
  });
}
...
function formatExplainLines(items: AnalysisEntry[]): string[] {
  ...
  function depth(x: number): number {
    ...
    return 2 + depth(lines[x]?.parent ?? 0);
  }
...
const getColumnSelector = (
  table: string,
  columnSelector: string | Array<string>,
): string => {
  if (typeof columnSelector === "string") return columnSelector;
  const [first = null, ...rest] = columnSelector;

  if (first === null) {
    throw new Error("column selector is empty");
  }

  return [
    `${table}.${escapeIdentifier(first)}`,
    ...rest.map(escapeIdentifier),
